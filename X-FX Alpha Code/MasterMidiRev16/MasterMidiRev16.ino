...
// Pot declarations 
#include <Bounce.h>
const int numReadings = 15; // Smoothing amount
...
int averagePlus[6] = {0,0,0,0,0}; // Pin average
int averageMinus[6] = {0,0,0,0,0}; // Pin average
...
void loop()
  ...
  for (int i = 0; i < pots; i++ )  // Get reading for all potentiometers
    ...
    for (int j = 0; j < numReadings; j++ )  // Loop to create average reading
       ...
       averagePlus[i] = average[i] + 5;
       averageMinus[i] = average[i] - 5;
       
       if (lastreading[i] <= averageMinus[i] && lastreading[i] >= averageMinus[i]) // If reading is different from the previous
            { 
            if (i < 1) // Hi-res pots
                {
