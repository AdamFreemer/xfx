...
boolean LowResPots = false; // Make TRUE if you add two low=res pots
...
void loop()    //  MAIN PROGRAM LOOP
{
  ...
  if (lastreading[i] != average[i])
        {
        usbMIDI.sendPitchBend(Converted, channel);
        }
...
if  (LowResPots) {    /// Code only executes if there are any 0-127 (Low Res) Pots
  ...
  if (LRP_lastreading[i] != LRP_average[i])
        {
        usbMIDI.sendControlChange(i, LRP_average[i], channel);  // Write out to USB
        }
  // Set last array value to current value
